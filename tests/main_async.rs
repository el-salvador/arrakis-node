...
#[tokio::test]
async fn connect_subscribe_read_note_and_match_enum_kind(){
  // Init Relay
  let ws_connection = NostrRelay::new(URL).await;

  ws_connection.subscribe(
    json!({"kinds":[300],"limit":1})
  ).await.expect("Failed to subscribe to relay!");
    ...
    Some(Ok(Message::Text(message))) => {
      if let Ok((_type, _id, note)) = from_str::<(String, String, SignedNote)>(&message) {
        match Kind::from_u32(note.kind) {
          Some(Kind::Code) => {
            if let Ok(code_note) = from_str::<Code>(&note.content) {
              // run code_note function for testing
              break;
            }
            break;
          },
          _ => panic!("Kind not found!")
        }
        break;
      }
    },
